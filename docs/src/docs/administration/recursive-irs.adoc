...
[IMPORTANT]
====
DISCLAIMER: THIS IS AN EXPERIMENTAL FEATURE INTRODUCED WITH APP VERSION 7.1.0 AND CHART VERSION 9.1.0.
====
...
. Create a CX-0152-compliant access policy and contract policy for the notification asset. The access policy controls catalog visibility; the contract policy defines the permitted use of the endpoint.
...
Access and contract policies must follow https://catenax-ev.github.io/docs/Saturn/standards/CX-0152-PolicyConstrainsForDataExchange[CX-0152 Policy Constraints for Data Exchange v1.0.0]. IRS does not prescribe one fixed business policy for every deployment; the provider offer and the consumer's accepted policy configuration must be compatible.

The following example shows the expected structure of an accepted policy. Replace the framework agreement and usage purpose placeholders with the values agreed for the PURIS data exchange. The provider's EDC contract policy must contain constraints that satisfy this accepted policy.
