...
----
If no custom policies are registered by REST, IRS will use default one configured with `irs-edc-client.catalog.acceptedPolicies` property. IRS will only negotiate contracts for offers with a policies found in Policy Store.
