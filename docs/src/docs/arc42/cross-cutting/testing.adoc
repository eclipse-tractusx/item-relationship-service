...
Recursive Cucumber end-to-end tests are located in https://github.com/eclipse-tractusx/item-relationship-service/blob/main/irs-cucumber-tests/src/test/resources/org/eclipse/tractusx/irs/cucumber/features/irs-recursive.feature[irs-recursive.feature] and cover complete results, grant filtering, missing grants and expired jobs in a configured multi-tier environment.
