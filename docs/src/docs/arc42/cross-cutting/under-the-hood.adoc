...
include::discovery-process-dtr.adoc[]
...
[plantuml,target=discovery-process-edc,format=svg]
----
@startuml
autonumber
participant EdcSubmodelClient
participant EdcOrchestrator
participant ContractNegotiationService
participant EdcControlPlane
participant ContractNegotiationIdStorage
participant EndpointDataReferenceStorage
participant EdcCallbackController
participant EdcDataPlaneClient

EdcSubmodelClient ->> EdcOrchestrator: Get EDR for asset and DSP endpoint
EdcOrchestrator ->> EndpointDataReferenceStorage: Get cached EDR by asset/DSP key
EndpointDataReferenceStorage -->> EdcOrchestrator: Return Optional<EDR>

alt Token is present and not expired
    EdcOrchestrator -->> EdcSubmodelClient: Return cached EDR
else
    alt EDR management mode
        EdcOrchestrator ->> ContractNegotiationService: Start new negotiation
    else Classic negotiation mode
        alt Token is expired
            EdcOrchestrator ->> ContractNegotiationService: Start transfer with existing agreement
        else Token is not present
            EdcOrchestrator ->> ContractNegotiationService: Start new negotiation
        end
    end
    ContractNegotiationService ->> EdcControlPlane: Start negotiation or transfer
    opt EDR management mode
        EdcControlPlane ->> EdcCallbackController: Negotiation callback
        EdcCallbackController ->> ContractNegotiationIdStorage: Map negotiation ID to agreement ID
        EdcOrchestrator ->> ContractNegotiationIdStorage: Poll for agreement ID
    end
    EdcControlPlane ->> EdcCallbackController: EDR callback
    EdcCallbackController ->> EndpointDataReferenceStorage: Store EDR by agreement ID
    loop While EDR is not present
        EdcOrchestrator ->> EndpointDataReferenceStorage: Poll by agreement ID
    end
    EndpointDataReferenceStorage -->> EdcOrchestrator: Return EDR
    EdcOrchestrator ->> EndpointDataReferenceStorage: Cache EDR by asset/DSP key
    EdcOrchestrator -->> EdcSubmodelClient: Return EDR
end
