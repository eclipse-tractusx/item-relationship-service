...
|*Recursive IRS*
|The *Recursive IRS* provides grant-controlled partner-to-partner traversal. It exposes the recursive job, notification and chain opening grant APIs, reads direct `SingleLevelBomAsPlanned` relationships, requests anonymized PURIS payloads through EDC and persists local job, correlation and grant state.
