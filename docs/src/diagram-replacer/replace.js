...
fs.readdirSync(TARGET_PATH).forEach((file) => {
  fs.readFile(TARGET_PATH + file, "utf8", (err, data) => {
    if (err) throw err;
      ...
      (image) => {
        ...
        if (path.parse(image).name === path.parse(file).name) {
          fs.rename(
            ...
            function (err) {
              if (err) {
                console.log("Error trying to read images: Access failed or no Images found. Error:")
                console.log(err);
    ...
    if (!fs.existsSync("./adocs-with-pngs")) {
      fs.mkdirSync("./adocs-with-pngs");
    }

    fs.writeFile(
      `./adocs-with-pngs/${file}`,
      output.join("\n"),
      "utf8",
      (err) => {
        if (err) throw err;
