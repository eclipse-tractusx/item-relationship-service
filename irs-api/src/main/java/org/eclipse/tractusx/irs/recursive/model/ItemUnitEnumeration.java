...
/** Units supported by the PURIS item quantity type. */
public enum ItemUnitEnumeration {
    ...
    ItemUnitEnumeration(final String value) {
        this.value = value;
    }
    ...
    @JsonCreator
    public static ItemUnitEnumeration fromValue(final String value) {
        return findByValue(value)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported item unit: " + value));
    }

    public static Optional<ItemUnitEnumeration> findByValue(final String value) {
        return Arrays.stream(values())
                     .filter(unit -> unit.value.equals(value))
                     .findFirst();
    }
}
