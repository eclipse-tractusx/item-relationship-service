...
/**
 * Uniquely identifies a chain opening grant. A node may hold several grants for the same
 * opening - one per requested material ({@code globalAssetId}) and requesting partner
 * ({@code requesterBpn}).
 */
public record RecursiveChainOpeningGrantKey(String openingId, String globalAssetId, String requesterBpn,
                                            RecursiveUseCase useCase) {

    public RecursiveChainOpeningGrantKey {
    ...
    public String toString() {
        return "openingId=" + openingId + ", globalAssetId=" + globalAssetId + ", requesterBpn=" + requesterBpn
                + ", useCase=" + useCase;
    }
}
