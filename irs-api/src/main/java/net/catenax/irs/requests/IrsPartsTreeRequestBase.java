...
public abstract class IrsPartsTreeRequestBase implements Serializable {
    ...
    @Parameter(description = "BoM Lifecycle of the result tree.", in = QUERY, example = "asBuilt",
               schema = @Schema(implementation = BomLifecycle.class))
    ...
    @Parameter(description = "AspectType information to add to the returned tree.", in = QUERY, explode = Explode.FALSE,
               example = AspectType.AspectTypesConstants.SERIAL_PART_TYPIZATION, array = @ArraySchema(
            schema = @Schema(implementation = AspectType.class,
                             defaultValue = AspectType.AspectTypesConstants.SERIAL_PART_TYPIZATION)))
