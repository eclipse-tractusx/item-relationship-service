...
public class ArtifactRequestController {
    ...
    public Response request(ArtifactRequestMessage message) {
        ...
        var dataRequest = DataRequest.Builder.newInstance().id(randomUUID().toString()).dataEntry(entry).dataDestination(dataDestination).protocol(IDS_REST)
                .properties((Map<String, String>) message.getProperties().get("dataspaceconnector-properties"))
                .build();
