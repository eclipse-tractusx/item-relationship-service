...
Feature: SingleLevelUsageAsBuilt
  ...
  Scenario: End 2 End for upward direction and SingleLevelUsageAsBuilt
    Given I register an IRS job for globalAssetId "urn:uuid:97eb9ea6-7e66-4ad8-aefe-6ed8aa78ccce" and BPN "BPNL00000003AVTH"
  ...
  Scenario: Tombstone for not found GlobalAssetId
    Given I register an IRS job for globalAssetId "urn:uuid:aaaaaaaa-bbbb-cccc-dddd-eeeeeeeeeeee" and BPN "BPNL00000003B5MJ"
    And depth 1
  ...
  Scenario: End 2 End for OEM-B (MB) [BPN:BPNL00000003AVTH] (SerialPart 3.0.0, SingleLevelBomAsBuilt 3.0.0 , Batch 3.0.0)
    Given I register an IRS job for globalAssetId "urn:uuid:4509ea7c-c8d2-41a1-83ca-c214ee34af6c" and BPN "BPNL00000003AVTH"
    And collectAspects "true"
    And depth 100
    And direction "downward"
    And aspects :
    ...
    And I check, if submodels contains BPNL number "BPNL00000003AVTH" exactly 37 times

  @DEV @INTEGRATION_TEST
  @TRI-704 @TRI-1941 @TRI-1942 @TRI-873 @TRI-1843 @TRI-1770 @TRI-1682
  Scenario: End 2 End for Tier A (ZF) [BPN:BPNL00000003B2OM] (SerialPart 3.0.0, SingleLevelBomAsBuilt 3.0.0 , Batch 3.0.0)
    Given I register an IRS job for globalAssetId "urn:uuid:aeada37a-a1d8-4535-a476-5f5e1142e3fe" and BPN "BPNL00000003B2OM"
    And aspects :
      | urn:samm:io.catenax.serial_part:3.0.0#SerialPart                          |
      | urn:samm:io.catenax.single_level_bom_as_built:3.0.0#SingleLevelBomAsBuilt |
      | urn:samm:io.catenax.batch:3.0.0#Batch                                     |
