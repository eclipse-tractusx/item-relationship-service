...
CLIENT_KEY="./keys/$CLIENT_NAME.key"
if [ -n "$3" ]; then
    ...
    openssl req -newkey rsa:2048 -new -batch -nodes -x509 -days 3650 -text -keyout "${CLIENT_KEY}" -out "$CLIENT_CERT"
fi
...
echo 'Creating client id: ' + $CLIENT_ID
